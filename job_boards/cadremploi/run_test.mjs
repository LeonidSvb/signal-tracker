// Cadremploi test runner — blackfalcondata/cadremploi-scraper
// Usage: node --env-file=../../../../../../.env run_test.mjs <test_name>
// Example: node --env-file=../../../../../../.env run_test.mjs directeur_agroa
...
console.log(`[${testName}] starting: keyword="${test.keyword}" maxItems=${test.maxItems}`);
console.log(`Note: postedDate is null for most listings — use firstSeenAt for cache-based stale detection`);
...
console.log(`\nSaved: ${rawPath}`);
console.log(`\nSample (first 5) — company = recruiter posting on behalf of food manufacturer:`);
for (const item of items.slice(0, 5)) {
  const firstSeen = item.firstSeenAt ?? "?";
  const isRepost = item.isRepost ? " [REPOST]" : "";
  console.log(`  [first:${String(firstSeen).slice(0,10)}]${isRepost} ${item.company ?? "?"} — ${item.title ?? "?"} @ ${item.location ?? "?"}`);
}
