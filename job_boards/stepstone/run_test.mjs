// StepStone test runner — memo23/stepstone-search-cheerio-ppr
...
console.log(`[${testName}] starting: keyword="${test.keyword}" location="${test.location}" maxItems=${test.maxItems}`);
...
for (const item of items.slice(0, 5)) {
  ...
  const anon = item.isAnonymous ? "[ANON]" : "";
  console.log(`  [${String(date).slice(0,10)}] ${anon}${item.companyName ?? "?"} — ${item.title ?? "?"} @ ${item.location ?? "?"}`);
}
