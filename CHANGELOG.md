...
### Fixed — LinkedIn AI hook+bridge, three independent copies of the HIRING routing bug, mixed-language copy, live 404s (2026-08-03/04)
Big session, client handoff prep. Summary:
- Ported AI hook+bridge to LinkedIn (`copyEngine.mjs`'s `buildLinkedInCopy`) — was 100% static, MA/EXPAND/INVEST/CONTRACT connection notes/first messages had unfilled `{acquirer}`/`{target}`/`{location}`. Added `linkedin.connection_note_mode` config (`with_note`/`no_note` A/B, per copy_templates.json's own long-planned note) so the first message doesn't verbatim-repeat the connection note's hook line.
- Found and fixed the SAME `signal_type` coarse-vs-granular routing bug (`HIRING` vs `HIRING_EXEC/MID/STALE/SURGE`) in THREE independent places: `build_linkedin_queue.mjs`, and two spots in the frontend (`nextjs/src/app/api/copy/route.ts` — the live "READY TO SEND" preview 404'd silently on every HIRING lead, >half of all volume).
- `/api/copy` never had AI generation at all ("no LLM call here" original design) — added it, matching the pipeline. Root cause of a live post-deploy bug (`{role_1}/{role_2}` shown verbatim): `OPENROUTER_KEY` was missing from Coolify's persistent env store (a prior hotfix never survived a rebuild) — added properly via Coolify API this time.
- Swept `copy_templates.json` for every unfillable `{acquirer}/{target}/{location}/{project}/{amount}/{CEO/MD}` placeholder (static fallback + followup emails, day 4/10/18/20 — actively pushed to PlusVibe by `route_email.mjs`, not dead code) and reworded around them.
- `market_focus`/`{relevant_case}` were substituted into DE/FR/NL messages AFTER translation, leaving one stray English word mid-sentence — wired in `translations.jsonl`'s existing (unused) `market_names` map + added a per-language relevant_case fallback.
- 136/138 stale `channel_actions` LinkedIn snapshots (pre-dating the AI fix, permanently cached) backfilled with real AI copy via a one-time script.
- `find_emails_leadsfriday.mjs`: chunked submissions (50-row orders instead of one giant one), self-heals orders that end without delivery (a LeadsFriday-side cancellation was permanently blocking 500 contacts), validates each found email via BounceBan immediately on delivery (event-driven, not the weekly cron).
- `useLeads.ts`: sidebar took 20-30s to load — filtered signals/contacts/contact_state/app_state queries to tiered company_ids only (~5x payload cut). Real remaining bottleneck is the self-hosted Supabase's inconsistent throughput, not app-side.
- Settings > Templates panel redesigned: a Universal skeleton block shown once + a lean per-type table (trigger rule + example hook/bridge), full messages one click away — the original one-row-per-type table read as repetitive since every type now shares the same wrapping skeleton.
- Added `--balance` to `leadsfriday_email_finder.py` (found the undocumented `/api/payments-summary/` endpoint).
...
- `pipeline/stages/classify_company.mjs`: the Q6 near-dup-clustering `sameEvent()` call (an accuracy nice-to-have, not essential) ran unprotected before the main per-signal classification loop — one hung OpenRouter request killed the entire run before any real classification happened, twice (2026-07-27, 2026-07-29). Wrapped in try/catch; a failed cluster now just skips that merge instead of aborting everything downstream.
- `pipeline/lib/companyClassifier.mjs` + `pipeline/stages/score_signals.mjs`: LLM calls used `httpRetry.mjs`'s default 20s timeout (tuned for fast REST calls, not a reasoning model with variable per-provider latency) — bumped to 60s. `score_signals.mjs` additionally used a raw `fetch()` with no timeout at all for two LLM calls; both now use `fetchRetry`.
- Recovery: ran the full chain manually end-to-end (`filter_icp` → `resolve_companies` → `find_contacts` → `classify_company` → `score_signals` → `rank_leads` → `build_signal_report`, `scrape_jobs`/`find_contacts_exa` skipped — Leo: don't spend new Apify/Exa on a backlog that's already scraped). Result: 491 backlogged Exa signals processed, 202 new `signals` rows created (closing the 250→1049 gap), 650 companies tiered (T1:85/T2:84/T3:26), 192 now visible on the frontend (`tier` set + ≥1 signal, per `useLeads.ts`'s filter).

### Fixed — `validate_contacts.mjs` fully on BounceBan, MV/Apify path retired (2026-07-31)
`pipeline/lib/validateEmail.mjs` used to run `scripts/validation/pipelines/mv-then-bounceban.py` (Million Verifier via Apify first, BounceBan only for the catch-all remainder) — the Apify account ran out of usage budget for the MV actor (`not-enough-usage-to-run-paid-actor`), silently producing 0 decisive verdicts every run. Switched to `scripts/validation/validate-catchall-bounceban.py` directly (despite the "catchall" name it validates any list) — verified live against 54 real pending emails (51 deliverable, 3 undeliverable) and the actual `validate_contacts.mjs --live` stage end to end.

### Added — `resolve_linkedin.mjs`, LinkedIn backfill via Exa (2026-07-31)
Real gap found: Blitz's own domain→LinkedIn match misses real, findable companies — a live 7/7 spot-check (Findus Switzerland, SARIA France, Maïsadour, Nexira, Upside Foods, Vitamin Well Group, Agristo) found a real LinkedIn company page on the first Exa query for every one, none had `companies.linkedin_url` set. Without it those companies can never become `find_contacts_exa.mjs` gap-mode candidates (that stage requires `linkedin_url`) — a closed loop that doesn't self-heal. New `resolveCompanyLinkedin()` in `pipeline/lib/exaFinder.mjs` (Exa search, word-boundary name-match guard — an early substring-match version wrongly linked "Back-Factory" to "Backyard Fun Factory"'s page) + `pipeline/stages/resolve_linkedin.mjs`, wired into `WEEKLY_CHAIN` right before `find_contacts_exa`. Backfilled 92 companies live (7 test + 90 backlog, 89% hit rate, ~$0.63 total).

### Fixed — Blitz API key confirmed dead (2026-08-01)
`BLITZ_API_KEY` returns 401 Invalid API key on every real endpoint (`/v2/search/waterfall-icp-keyword`, `/v2/enrichment/email`) — not a budget/rate issue, the key itself is invalid. This silently zeroed out `find_contacts.mjs` (person-finding) and the Blitz half of `find_contacts_exa.mjs` (email lookup) for an unknown period before discovery — both stages kept reporting `status: success` since neither call site throws on a JSON auth-error response, just returns null/no-match. Root cause not fixed (Leo: budget not available to renew right now) — `classify_company.mjs`/`resolve_companies.mjs`'s existing Layer0(free sourcing lookup)/Layer1(Blitz→Exa+LLM fallback) cascade already degrades gracefully with no code changes needed (confirmed live: recent runs show `layer1BlitzResolved` near-zero, `layer1ExaLlm`/`layer1Resolved` carrying the load, real pass/reject output unaffected in kind, only in evidence quality).

### Added — LeadsFriday `waterfall_email_finder` wired in as the primary email-finding path (2026-08-02)
With Blitz dead, `find_contacts_exa.mjs`'s own email step (Blitz `/v2/enrichment/email`) is dead too — LeadsFriday becomes primary, not a fallback. New `pipeline/stages/find_emails_leadsfriday.mjs`, two-phase per run (matching the API's hours-long async turnaround): **check** every pending `runs/find_emails_leadsfriday_*` dir (checkpoint.json present, manifest.json not yet — same triad convention as every other stage) and patch delivered results into `contacts.email`; **submit** new candidates (linkedin known, email missing, not already in-flight). Wraps the existing `scripts/enrichment/leadsfriday_email_finder.py` (not porting its polling/download logic to JS, same rule as the BounceBan wrapper). Wired into `WEEKLY_CHAIN` after `find_contacts_exa`.
- Real numbers from a live 7-contact test: 57% hit rate (4/7, vs. this account's historical blended 46.7% across 3 older mixed-niche batches), ~0.0125 credits per *found* email (stable across batches).
- LeadsFriday reserves credits per REQUESTED row up front, not per found result (refunded after delivery) — a 613-row submit was rejected needing 7.36 credits reserved with only 6.29 available, even though the real post-refund cost would have fit. `find_emails_leadsfriday.mjs` handles this by halving the batch and retrying (no balance-check endpoint exists — confirmed live, every guessed `/api/*credits*`/`/api/*wallet*`/`/api/*account*` path 404s) instead of just failing, and reports an explicit "top up at app.leadsfriday.com" message (console + `pipeline_runs.errors` + manifest) when even a 10-row batch won't fit, per Leo's explicit ask that a balance shortfall be visible, not silent.
- LeadsFriday also proxies Apollo (`platform_key: "apollo"`, filtered to `contactEmailStatusV2=verified`) but only via their own web dashboard — no submit API found for it (every guessed endpoint 404s), so it stays a manual-only path, not automated here.
- Submitted order #83 (500 contacts, the largest batch the available balance covered) — 113 contacts held back for a future run once balance allows.

### Discovered — sourcing.companies TAM base status, 2026-08-02
32,359 companies total (Layer 0 free-match cache, separate `sourcing` schema, same Supabase instance). 12,199 ever classified (pass 4,585 / reject 7,561 / needs_website 53), 20,160 (62%) never scored — mostly `blitz_tam_004`'s bulk scrape, sitting with real cached `employees`/`industry`/often `about` data already but no ICP verdict. Decision: NOT bulk-prescoring these now — this system is signal-reactive by design (Layer0/1/2 cascade classifies lazily, exactly once, permanently cached), and most of the 20k will likely never fire a real signal in the monitored window; eager classification would spend real money on companies that may never be used. Revisit only if the product ever needs a precomputed list for proactive/blast outreach instead of signal-triggered contact.
